...
function clipTraceText(value, maxLength = 160) {
...
function formatObserverSnapshotForTrace(observer) {
  const snapshot = observer?.snapshot ?? {};
  return [
    `bridgeState=${snapshot.bridgeState ?? "unknown"}`,
    `codexState=${snapshot.codexState ?? "unknown"}`,
    `busy=${snapshot.busy === true ? "true" : "false"}`,
    `promptReady=${snapshot.promptReady === true ? "true" : "false"}`,
    `turnSettled=${snapshot.turnSettled === true ? "true" : "false"}`,
    `modalChoiceArmed=${snapshot.modalChoiceArmed === true ? "true" : "false"}`,
    `lastEvent=${snapshot.lastEvent ?? "none"}`,
    `sessionId=${snapshot.sessionId ?? "none"}`
  ].join(", ");
}

function formatRecentObserverEvents(observer, minEventIndex = -1, limit = 6) {
  const events = Array.isArray(observer?.events) ? observer.events : [];
  const recent = events
    .filter((event) => event.index > minEventIndex)
    .slice(-limit)
    .map((event) => {
      const text = clipTraceText(event.text ?? event.sessionId ?? "", 80);
      return text
        ? `${event.index}:${event.type}:${JSON.stringify(text)}`
...
function buildNativeTaskSubmissionTimeoutDetail(observer, {
  minEventIndex = -1,
  retries = 0,
  task = null
} = {}) {
  const snapshot = observer?.snapshot ?? {};
  const taskSummary = clipTraceText(task, 120) ?? "<empty>";
  const lastText = clipTraceText(snapshot.lastText, 120) ?? "none";
  const recentText = clipTraceText(snapshot.recentText, 200) ?? "none";
  const recentEvents = formatRecentObserverEvents(observer, minEventIndex);
  return `task=${JSON.stringify(taskSummary)}; retries=${retries}; snapshot={${formatObserverSnapshotForTrace(observer)}}; lastText=${JSON.stringify(lastText)}; recentText=${JSON.stringify(recentText)}; recentEvents=[${recentEvents}]`;
}
...
function detectPromptReady(normalizedText) {
  ...
  if (/(^|\s)(?:›|>)\s*(?:\/|[A-Za-z0-9])/.test(tail)) {
    return true;
  }
  return false;
}

function detectInteractableCodexBanner(text) {
...
function isTaskEchoEvent(event, task) {
  if (!event || (event.type !== "prompt-ready" && event.type !== "native-output")) {
    return false;
  }
...
export async function sendNativeTaskWithRetry({
  ...
  try {
    ...
    const startupBannerVisible = detectInteractableCodexBanner(
      `${observer?.snapshot?.recentText || ""}\n${observer?.snapshot?.lastText || ""}`
    );
    const canUseDelayedPromptFallback = message.includes("Timed out waiting for native Codex prompt readiness")
      && (observer?.snapshot?.codexState === "waiting-for-prompt" || startupBannerVisible)
      && !observer?.snapshot?.modalChoiceArmed;
    if (!canUseDelayedPromptFallback) {
      throw error;
    }
    observer.record("task-await-ready-fallback", {
      reason: startupBannerVisible ? "startup-banner-visible" : "prompt-detection-delayed",
      text: observer.snapshot.lastText || "prompt-detection-delayed"
    });
    onProgress?.(`native-task=await-ready-fallback | reason=${startupBannerVisible ? "startup-banner-visible" : "prompt-detection-delayed"}`);
  }
  ...
  while (true) {
    let candidate;
    try {
      candidate = await waitForObserverEvent(
        observer,
        (event) => event.index > minEventIndex
          && (event.type === "native-busy"
            || event.type === "prompt-ready"
            || (controller.mode === "pty" && event.type === "native-output")),
        timeoutMs,
        "Timed out waiting for native task submission to start."
      );
    } catch (error) {
      const detail = buildNativeTaskSubmissionTimeoutDetail(observer, {
        minEventIndex,
        retries: submitRetries,
        task: normalizedTask
      });
      observer.record("task-submit-timeout", {
        retries: submitRetries,
        text: detail
      });
      onProgress?.(`native-task=timeout | phase=submit-start | retries=${submitRetries}`);
    ...
    if ((candidate.type === "prompt-ready" || (controller.mode === "pty" && candidate.type === "native-output"))
      && isTaskEchoEvent(candidate, normalizedTask)) {
      ...
      observer.record("task-submit-retry", {
        retries: submitRetries,
        sourceEventType: candidate.type,
        text: candidate.text ?? normalizedTask
      });
      onProgress?.(`native-task=retry-submit | retry=${submitRetries} | source=${candidate.type}`);
...
export class NativeRemoteSession {
  ...
  async start({ onProgress = null } = {}) {
    ...
    this.codexChild = usePty
      ? spawnPty(command[0], command.slice(1), {
          cwd: this.dir,
          env: process.env,
          cols: this.cols ?? (process.stdout.columns || 120),
          rows: this.rows ?? (process.stdout.rows || 40),
    ...
    if (usePipe) {
      attachObservedPipe(this.codexChild, "stdout", process.stdout, this.observer, {
        forwardOutput: this.forwardOutput,
        outputHandler: this.onOutputChunk
      });
      attachObservedPipe(this.codexChild, "stderr", process.stderr, this.observer, {
        forwardOutput: this.forwardOutput,
        outputHandler: this.onOutputChunk
      });
    }
    if (usePty) {
      attachObservedPty(this.codexChild, process.stdout, this.observer, {
        forwardOutput: this.forwardOutput,
        outputHandler: this.onOutputChunk
      });
    }
    ...
    // Some real Codex boots can take longer or delay prompt detection even though the
    // native session is otherwise alive, so Electron host should degrade to a soft
    // warning instead of failing the whole session start.
    try {
      ...
      const message = String(error?.message || error || "");
      if (!message.includes("Timed out waiting for native Codex prompt readiness")) {
        throw error;
      }
      onProgress?.("native-prompt=delayed | continuing with live session");
      this.observer.markCodexState("waiting-for-prompt", { text: "prompt-detection-delayed" });
    }
  ...
  resize(cols, rows) {
    if (this.stdioMode !== "pty" || typeof this.codexChild?.resize !== "function") {
      return false;
    }
    this.cols = cols;
    this.rows = rows;
    try {
      this.codexChild.resize(cols, rows);
      return true;
    } catch (error) {
      const message = String(error?.message || error || "");
      const code = String(error?.code || "");
      if (code === "EBADF" || /EBADF|ioctl\(2\) failed/i.test(message)) {
        return false;
      }
      throw error;
    }
...
function attachObservedPipe(child, streamName, targetStream, observer, { forwardOutput = false, outputHandler = null } = {}) {
  if (!child[streamName]) {
    return;
  }
  ...
  child[streamName].on("data", (chunk) => {
    ...
    outputHandler?.(chunk, { source: streamName });
...
function attachObservedPty(child, targetStream, observer, { forwardOutput = false, outputHandler = null } = {}) {
  child.onData((chunk) => {
    observer.ingestChunk("pty-output", chunk);
    outputHandler?.(chunk, { source: "pty-output" });
    if (forwardOutput && targetStream) {
      targetStream.write(chunk);
    }
  });
...
export async function launchNativeCodexSession({
  ...
  const envForward = process.env.QUICK_CODEX_WRAP_NATIVE_FORWARD_OUTPUT;
  const resolvedForwardOutput = forwardOutput != null
    ? forwardOutput
    : envForward != null
      ? envForward === "1"
      : stdioMode === "pty"; // PTY sessions are TUI-like; pipe sessions default to silent.
  ...
  try {
    ...
    if (usePipe) {
      attachObservedPipe(codexChild, "stdout", process.stdout, observer, { forwardOutput: resolvedForwardOutput });
      attachObservedPipe(codexChild, "stderr", process.stderr, observer, { forwardOutput: resolvedForwardOutput });
    }
    if (usePty) {
      attachObservedPty(codexChild, process.stdout, observer, { forwardOutput: resolvedForwardOutput });
    }
