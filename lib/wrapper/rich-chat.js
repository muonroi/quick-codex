...
function responseBody(response) {
  ...
  if (response.final?.text) {
    return String(response.final.text).trim();
  }
...
function ResponsePane({ response, busy, mode = "short", scrollOffset = 0, viewportLines = 12, height = null }) {
  ...
  const finalText = response ? responseBody(response) : "";
  const preview = response && mode !== "full"
    ? truncateWithTail(finalText, 14, 14)
    : { text: finalText, truncated: false };

  // Keep the panel height stable by always rendering within a line window.
  const windowed = sliceTextLines(preview.text, scrollOffset, viewportLines);

  const body = response
    ? `${windowed.window}${mode !== "full" && preview.truncated && response.final?.outputPath ? `\n\nFull output: ${response.final.outputPath}` : ""}`
    : "Submit a task to see the routed result, trace, and final output here.";
  return h(
    Box,
    { flexDirection: "column", borderStyle: "round", borderColor: "magenta", paddingX: 1, height },
    h(Text, { color: "magentaBright", bold: true }, "Result"),
    h(Text, { color: busy ? "yellow" : "green", wrap: "truncate" }, summary),
    traceLine ? h(Text, { color: "cyan", wrap: "truncate" }, `Trace: ${traceLine}`) : null,
    artifactLine ? h(Text, { color: "yellow", wrap: "truncate" }, `Artifact: ${artifactLine}`) : null,
    response ? h(Text, { color: "gray", bold: true }, mode === "full" ? "Final (full):" : "Final (preview):") : null,
    response
      ? h(
          Text,
          { dimColor: true, wrap: "truncate" },
          `Scroll: PgUp/PgDn or Ctrl+Up/Ctrl+Down (${windowed.offset + 1}-${Math.min(windowed.offset + viewportLines, windowed.total)} of ${windowed.total})`
        )
      : null,
...
function App({ session, bannerLines, sessionName }) {
  ...
  return h(
    ...
    h(ResponsePane, { response: lastResponse, busy, mode: resultMode, scrollOffset: resultScroll, viewportLines, height: responseHeight }),
    h(
      Box,
