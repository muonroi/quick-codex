...
function ResponsePane({ response, busy, mode = "short" }) {
  ...
  const preview = response
    ? (mode === "full" ? truncateLines(finalText, 80) : truncateWithTail(finalText, 8, 10))
    : { text: "", truncated: false };
  const body = response
    ? `${preview.text}${preview.truncated && response.final?.outputPath ? `\n\nFull output: ${response.final.outputPath}` : ""}`
  ...
  return h(
    ...
    traceLine ? h(Text, { dimColor: true, wrap: "truncate" }, traceLine) : null,
    artifactLine ? h(Text, { dimColor: true, wrap: "truncate" }, artifactLine) : null,
    response ? h(Text, { color: "gray" }, "Final:") : null,
...
function Header({ sessionName, bannerLines }) {
  return h(
    ...
    ...bannerLines.map((line, index) => h(Text, { key: `banner-${index}`, dimColor: true, wrap: "truncate" }, line))
  );
}
...
function App({ session, bannerLines, sessionName }) {
  ...
  return h(
    ...
    h(ResponsePane, { response: lastResponse, busy, mode: resultMode }),
    h(
      Box,
