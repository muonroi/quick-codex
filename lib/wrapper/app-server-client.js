...
function isOversizedResumeError(error) {
  const text = String(error?.message ?? error ?? "");
  return /thread\/resume/i.test(text)
    && (/space limit exceeded/i.test(text) || /message too long/i.test(text));
}
...
export class CodexAppServerSession {
    ...
    if (nativeAction === "thread/resume") {
      ...
      if (this.activeThreadId !== threadId) {
        try {
          ...
          if (!isOversizedResumeError(error)) {
            throw error;
          }
          ...
          this.lastThreadFallback = {
            from: "thread/resume",
            to: "thread/start",
            reason: "thread-resume-payload-too-large",
  ...
  async runDecision({ dir, decision, policy, outputLastMessage = null }) {
    ...
    try {
      ...
      if (nativeAction === "thread/compact/start" && this.lastThreadFallback?.to !== "thread/start") {
      ...
      return {
        ...
        nativeThreadActionEffective: this.lastThreadFallback?.to ?? nativeAction,
        nativeThreadFallback: this.lastThreadFallback,
        threadId: this.activeThreadId,
