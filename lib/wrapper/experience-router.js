import { routeTask as classifyLocalTaskRoute } from "./task-router.js";
import fs from "node:fs";
import os from "node:os";
import path from "node:path";

const EXPERIENCE_CONFIG_PATH = path.join(os.homedir(), ".experience", "config.json");
const DEFAULT_MODEL_ROUTER_TIMEOUT_MS = 6000;
const BROAD_MODEL_ROUTER_TIMEOUT_MS = 12000;
...
function inferModelRoutingTimeoutMs({ taskText, decision = null }) {
  const explicitTimeout = parsePositiveInt(process.env.QUICK_CODEX_WRAP_MODEL_ROUTER_TIMEOUT_MS);
  if (explicitTimeout) {
    return explicitTimeout;
  }

  let routeHint = decision?.route ?? null;
  if (!routeHint && taskText) {
    try {
      routeHint = classifyLocalTaskRoute({ task: taskText }).route;
    } catch {
      routeHint = null;
    }
  }

  if (routeHint === "qc-flow") {
    return BROAD_MODEL_ROUTER_TIMEOUT_MS;
  }

  return DEFAULT_MODEL_ROUTER_TIMEOUT_MS;
}
...
export async function resolveExperienceModelRoute({ dir, task = null, artifact = null, decision = null }) {
  ...
  const timeoutMs = inferModelRoutingTimeoutMs({ taskText, decision });
